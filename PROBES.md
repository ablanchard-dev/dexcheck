...
- **Every Windows account, not only the one running the check**: browser history, PowerShell history, WER crash reports and the known-cheat folder scan read each user profile on the machine (admin needed for the other accounts). Registry traces of an account that is not signed in (its `NTUSER.DAT` is not loaded) stay out of reach: reading them would mean loading a hive, which the check never does
